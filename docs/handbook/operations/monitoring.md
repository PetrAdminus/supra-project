...
| Status | `status_overview.canceled` | Количество отменённых лотерей | > 0 без активного плана рефанда |
...
| Refund | `refund_batch_pending` | Адресов в очереди возврата (по view `status_overview`) | > 0 дольше 4 часов |
| Refund | `refund_sla_breach` | Флаг нарушения SLA возврата | `true` |
...
- **View `list_automation_bots`:** периодический опрос (каждые 60 секунд для внутренних панелей) для отображения pending-действий, лимитов `max_failures` и таймлоков AutomationBot; при ручных проверках используйте `./supra/scripts/automation_status.sh <config> list|get` для оперативного снимка через Supra CLI.
