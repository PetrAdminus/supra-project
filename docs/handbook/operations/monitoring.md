...
- **View `lottery_engine::automation::registry_snapshot`:** периодический опрос (каждые 60 секунд для внутренних панелей) для отображения pending-действий, лимитов `max_failures` и таймлоков AutomationBot; при ручных проверках используйте `./SupraLottery/supra/scripts/automation_status.sh <config> list|get`, который проксирует view `lottery_engine::automation::registry_snapshot` и `lottery_engine::automation::bot_status`. На основе этого view рассчитывается индикатор `automation_timelock_breach`, сравнивающий таймлок с порогом 900 секунд для чувствительных действий.
