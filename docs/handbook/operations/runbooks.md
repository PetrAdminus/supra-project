...
> Дополнительные документы: [чек-лист релиза](release_checklist.md), [мониторинг](monitoring.md), [журнал операций](incident_log.md), [программа баг-баунти](bug_bounty.md), [процедура рефанда](refund.md), [пострелизная поддержка](post_release_support.md), [шаблон постмортема](postmortems.md).
...
4. Для импортированных розыгрышей используйте `./supra/scripts/history_backfill.sh`:
   - `dry-run <summary_path> [--lottery-id <id>] [--hex-output path] [--hash-output path]` — рассчитывает sha3-256 хэш файла BCS (поддерживаются бинарные и hex-представления) и формирует готовую команду `import`; команда использует Python-утилиту `supra.tools.history_backfill_dry_run` и позволяет заранее сохранить hex/хэш в артефакты CI.
...
- Использовать `./supra/scripts/refund_control.sh <config> cancel <lottery_id> <reason_code> <timestamp>` для перевода статуса и фиксации `CancellationRecord`.
- Запускать батчи возвратов через `./supra/scripts/refund_control.sh <config> batch <lottery_id> <round> <tickets> <prize_refund> <operations_refund> <timestamp>`.
...
- По завершении вызвать `./supra/scripts/refund_control.sh <config> archive <lottery_id> <finalized_ts>` и убедиться, что `summary` содержит статус `STATUS_CANCELED`.
