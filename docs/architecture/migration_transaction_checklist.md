...
| 4 | Инициализация `lottery_data` (публикация `lottery_state`, `instances`, `rounds`, `operators`, `treasury`, `treasury_multi`) | Админский аккаунт | Таблица соответствий заполнена, значения параметров подтверждены | Появились базовые ресурсы `lottery_data::LotteryState`, `InstanceRegistry`, `RoundRegistry`, `OperatorRegistry`, `Treasury`, `TreasuryMulti` |
| 5 | Миграция данных `lottery_core` → `lottery_data` (`migrate_rounds`, `migrate_instances`, `migrate_treasury`) | Админский аккаунт | Проверка `RoundCollection`, `LotteryCollection`, `Vaults` присутствуют и не пустые | Новые ресурсы содержат копии данных, старые помечены как архивные |
...
| 11 | Перенос записей отмен: запустить скрипт `migrate_cancellations`, который сначала вызывает `lottery_data::cancellations::import_existing_cancellation(s)` с payload'ами `LegacyCancellationRecord`, а затем dry-run `lottery_engine::cancellation::cancel_lottery` для новых кейсов и сверяет суммы продаж/джекпота | Админский аккаунт | Таблица соответствий обновлена, выгружены данные `lottery_multi::lottery_registry` (причины, суммы) или dry-run отчёты | Новый `CancellationLedger` содержит записи с корректными суммами, события отмен зафиксированы, pending-запросы очищены |
...
| 18 | Обновление VRF-контура: импортировать историческое состояние депозита через `lottery_data::vrf_deposit::import_existing_ledger` (payload `LegacyVrfDepositLedger`), затем применить `lottery_engine::vrf_config` (gas-бюджеты, whitelists, client seed), обновить `lottery_engine::vrf` (снапшот депозита, паузы) и синхронизировать `lottery_vrf_gateway` / `SupraVrf` | Админский аккаунт + скрипт VRF | Подготовлены payloadы из devnet/testnet архивов или dry-run отчётов, обновлены значения в `lottery_data::lottery_state` и `lottery_data::vrf_deposit`, события `Vrf*Updated` и `VrfDeposit*` зафиксированы, баланс депозита достаточен | Новые параметры активны, dry-run цепочки `schedule → request → fulfill` через `lottery_engine::draw` проходит успешно, пауза депозита соответствует импортированному состоянию |
...
| 2025-12-25 | Шаг 11 дополнен использованием payload'ов `LegacyCancellationRecord` и admin-entry `lottery_data::cancellations::import_existing_cancellation(s)`, чтобы импортировать исторические отмены перед dry-run `lottery_engine::cancellation`; обновлены входные проверки. | Supra refactoring bot |
| 2025-12-26 | Шаг 18 дополнен импортом `LegacyVrfDepositLedger` через `lottery_data::vrf_deposit::import_existing_ledger`, чтобы синхронизировать конфигурацию и баланс VRF-депозита перед обновлением `lottery_engine::vrf_config`/`lottery_engine::vrf`; обновлены требования к входным данным и ожидаемый результат. | Supra refactoring bot |
