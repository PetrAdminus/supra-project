...
**Как обновлять:** запустите `python docs/architecture/tools/export_move_inventory.py` из корня репозитория. При необходимости можно указать иные пути через аргументы `--workspace-root` и `--output`. Для автоматизированных сверок используйте JSON-экспорт (`--json-output docs/architecture/move_struct_inventory.json`), формат которого описан в `docs/architecture/move_struct_inventory_schema.md` и включает `schema_version`, счётчики пакетов/модулей/структур и рекурсивные записи «пакеты → модули → структуры».
