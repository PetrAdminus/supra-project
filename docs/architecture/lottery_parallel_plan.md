...
- **Алгоритм сопоставления.** После получения VRF seed модуль `winner_picker::compute` конвертирует его в последовательность 256-битных слов (через SHA3-256 от `seed || slot_index || schema_version`). Для каждого слота берётся следующее слово и нормализуется: `winner_index = random_word % total_tickets`. Если `winners_dedup = true`, используется структура `UsedTicketBitmap`; при коллизии выполняется следующий хэш `SHA3-256(previous_hash)` с ограничением числа повторов (по умолчанию 16). Если лимит попыток исчерпан, включается резервная стратегия, выбранная `draw_algo` (например, stride). Для варианта со `stride` документируется математическое обоснование равномерности (в `docs/handbook/architecture/winner_picker.md`) и добавляются property-тесты, которые сравнивают распределение с off-chain эталоном для малых `total_tickets`, чтобы исключить bias. Таким образом, распределение остаётся равномерным и не зависит от порядка слотов.
- **Идемпотентность батчей.** Результаты вычислений пишутся в таблицу `WinnerAssignment { lottery_id, slot_id, ticket_ref, winner_record_hash }` и сопровождаются событием `WinnerComputed`, куда дополнительно попадает `cursor_from/cursor_to`, `attempt`, агрегированный `winners_batch_hash` и `checksum_after_batch` (контрольная сумма на подмножество записей). Повторный запуск батча проверяет наличие записи, совпадение хэша `winner_record_hash = SHA3-256(seed || slot_id || ticket_ref || schema_version)`, соответствие `checksum_after_batch` и прогресс `WinnerCursor`. Несоответствие приводит к `abort(E_WINNER_MISMATCH)` и переходу в аварийный режим. Порядок обработки слотов фиксируется заранее (по индексу в векторе), что гарантирует детерминизм на всех узлах.
...
- `WinnerComputed(event_version, event_category, id, batch_no, winners_batch_hash, checksum_after_batch, processed_count, cursor_from, cursor_to)`
- `PayoutBatch(event_version, event_category, id, batch_no, paid, total, asset_type_code, cursor_from, cursor_to, payout_nonce)`
...
- `PriceFeedClampTriggered(event_version, event_category, asset_id_code, old_price, new_price)`
...
- **Fallback и проверки.** План предусматривает резервный агрегатор: при недоступности основного фида система активирует fallback, публикует событие `PriceFeedFallbackActivated` и уведомляет фронтенд. Все операции проверяют `staleness_window` (по умолчанию ≤ 300 секунд); при устаревших данных покупка билетов и резервирование призов немедленно завершаются `abort(E_PRICE_STALE)`. Fallback допускается только для информационных view/UI, без выполнения финансовых операций, о чём отдельно сообщается событием `PriceFeedFallbackActivated`. Для защиты от аномалий вводится «кламп» (`|Δ| > PRICE_JUMP_THRESHOLD` ⇒ `PriceFeedClampTriggered` + блокировка покупок) и таблица `price_decimals`, описанная в `docs/handbook/architecture/price_feeds.md`, чтобы избежать ошибок округления (например, $0.0099). Все тесты покрывают границы округления и сценарии аномального скачка цены.
...
- **Спецификации.** Для ключевых модулей `registry`, `economics`, `draw`, `payouts`, `history` и `feature_switch` добавляется папка `supra/move_workspace/sources/lottery_multi/spec/` с `*.move`-спеками. Обязательные утверждения: неизменность `snapshot_hash` и `slots_checksum`, невозможность двойного резервирования джекпота, монотонность `payout_round`, идемпотентность `WinnerAssignment`, корректность `FeatureSwitch` (режимы не переходят напрямую из `Disabled` в `PremiumOnly` без промежуточного `EnabledAll`).
