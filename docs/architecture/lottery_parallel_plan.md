...
- **Пакеты** — создаём новый `lottery_multi`, разбитый на тонкие модули (`registry`, `sales`, `draw`, `payouts`, `history`,
  `views`, `roles`, `economics`, `feature_switch`, `tags`, `legacy_bridge`) и использующий мосты к существующим `lottery_core`,
...
- **Текущая кодовая база.** Контракты расположены в репозитории `SupraLottery` и собраны в три Move-пакета — `lottery_core`, `lottery_support`, `lottery_rewards`. Дополнительные скрипты управления находятся в каталоге `SupraLottery/supra/scripts`, фронтенд — в `frontend/`.
...
### 3.1 Пакеты Move
- **Новый пакет `lottery_multi`.** Он разбит на «тонкие» модули (`registry`, `sales`, `draw`, `payouts`, `history`, `views`, `roles`, `economics`, `feature_switch`, `tags`, `legacy_bridge`), чтобы каждый файл оставался значительно ниже лимита 60 000 байт. Общие типы и константы выносятся в `lottery_multi::types`, валидации — в `lottery_multi::validators`, а базовые классификаторы находятся в новом модуле `lottery_multi::tags`. Пакет зависит от `lottery_core` (повторное использование библиотек работы с билетами, сейфами и комиссией) и `lottery_support` (существующие структуры истории и миграций). Дополнительно следует задокументировать зависимости в `SupraLottery/supra/move_workspace/Move.toml`.
...
- **Повторные попытки.** `VrfState.retry_after_ts` задаёт момент, когда разрешён новый запрос, а `retry_strategy` (0 — фиксированный интервал, 1 — экспоненциальный рост, 2 — ручной контроль администратором через `draw::schedule_manual_retry_admin`) определяет, как AutomationBot вычисляет следующий дедлайн. Число попыток ограничено параметром `MAX_VRF_ATTEMPTS` (в релизе v1 — 5); его превышение автоматически переводит лотерею в `Canceled` и инициирует возврат средств.
...
- `RefundBatchEvent(event_version, event_category, id, refund_round, tickets_refunded, prize_refunded, operations_refunded, total_tickets_refunded, total_amount_refunded)`
...
3. **Обновление CLI/скриптов.** Python-утилиты (`SupraLottery/supra/scripts`) нужно адаптировать к новым видам запросов, добавить команды для управления множеством лотерей (создание, пауза, батчи выплат, архив). В документации указать новые флаги и параметры.
...
19. **Ончейн-прайс-фиды и поддержка нескольких активов Supra.** Используем модуль `lottery_multi::price_feed` как обёртку над Supra Price Oracle. Реестр `PriceFeedRegistry` хранит `version`, таблицу фидов (ключ — `asset_id` типа `u64`) и EventHandle’ы (`PriceFeedUpdatedEvent`, `PriceFeedFallbackEvent`, `PriceFeedClampEvent`, `PriceFeedClampClearedEvent`). Запись фида (`PriceFeedRecord`) включает цену в целочисленной форме, `decimals`, `staleness_window`, `clamp_threshold_bps`, признак fallback и активность клампа. Базовые константы — `DEFAULT_STALENESS_WINDOW = 300 сек`, `DEFAULT_CLAMP_THRESHOLD_BPS = 2_000`, предопределённые активы `ASSET_SUPRA_USD = 1`, `ASSET_USDT_USD = 2`. Любое обновление, регистрация и активация fallback сопровождаются событиями. Абсолютная смена цены свыше порога запускает `PriceFeedClampEvent`, устанавливает `clamp_active = true` и требует ручного подтверждения `clear_clamp` (с событием `PriceFeedClampClearedEvent`), иначе `latest_price` завершится `abort(E_PRICE_CLAMP_ACTIVE)`. Детальные процессы описаны в [docs/handbook/architecture/price_feeds.md](../handbook/architecture/price_feeds.md).
...
- **Статусная страница.** План подразумевает запуск публичного статуса сервиса с историей инцидентов и текущим состоянием (продажи, VRF, выплаты). View `lottery_multi::views::status_overview` (см. `docs/handbook/operations/status_page.md`) агрегирует необходимые показатели для дашборда; ссылка на страницу добавляется в `README.md` и «книгу проекта».
- **Клиентские интеграции.** Фронтенд вызывает `/lottery-multi/views` через `supraClient.fetchLotteryMultiViewsSupra`, приводя поля `status_overview` к camelCase и переиспользуя mock `frontend/src/mocks/lottery-multi-views.json` для локальных сценариев.
- **Клиентские интеграции.** Фронтенд вызывает `/lottery-multi/views` через `supraClient.fetchLotteryMultiViewsSupra`, приводя поля `status_overview` к camelCase и переиспользуя mock `frontend/src/mocks/lottery-multi-views.json` для локальных сценариев. React Query-хук `useLotteryMultiViews` подключает агрегированные показатели к `components/Dashboard.tsx`, обновляя карточки и таблицы каждые 30 секунд.
...
- **Ончейн-реестр `vrf_deposit`.** Модуль `lottery_multi::vrf_deposit` хранит конфигурацию порогов (`min_balance_multiplier_bps`, `effective_floor`), актуальное состояние депозита и выпускает события `VrfDepositSnapshot`, `VrfDepositAlert`, `VrfRequestsPaused`, `VrfRequestsResumed`. Функция `ensure_requests_allowed` вызывается перед `request_vrf` и блокирует новые запросы, если `effective_balance` ниже порога.
...
- **Runbook отмены.** В `docs/handbook/operations/refund.md` описывается процедура `Canceled/Refund`: сроки запуска батчей (не позднее 24 ч после решения), размер партии (по умолчанию 200 выплат), событие прогресса `RefundBatchEvent` с накопительными агрегатами и отображение статуса во фронтенде. Коды причин отмены (`reason_code`) сопоставляются с человекочитаемой матрицей, которую видит пользователь. Завершение процедуры фиксируется вызовом `payouts::archive_canceled_lottery_admin`, который проверяет полноту рефандов и сохраняет `LotterySummary` со статусом `STATUS_CANCELED`.
...
- **Этап 4 — миграция и backfill.**
  ...
  - ✅ (2025-11-25) Реализованы on-chain функции `history::{import_legacy_summary_admin, rollback_legacy_summary_admin, update_legacy_classification_admin}`, события `LegacySummaryImported/RolledBack/ClassificationUpdated` и view `is_legacy_summary`.
  - ✅ (2025-11-25) Добавлены Move-тесты `history_migration_tests` и CLI `SupraLottery/supra/scripts/history_backfill.sh`, покрывающие импорт, откат и ручное назначение тегов.
  - ✅ (2025-11-28) Добавлен dry-run (`history_backfill.sh dry-run`) с Python-утилитой `supra.tools.history_backfill_dry_run`, формирующей hex/sha3 и подсказку команды `import`; поведение покрыто pytest `tests/test_history_backfill_dry_run.py`, runbook и release checklist дополнены инструкциями по сохранению артефактов.
...
- **Этап 5 — запуск ядра под управлением администратора.**
  ...
  - ✅ (2025-11-24) Реализованы события выдачи/отзыва капабилити (`PayoutBatchCapGranted/Revoked`, `PartnerPayoutCapGranted/Revoked`, `PremiumAccessGranted/Revoked`), вьюхи `list_partner_caps`/`list_premium_caps`, административные функции `cleanup_expired_admin`, `revoke_*` и тесты `roles_tests` для контроля expiry и журналирования.
  ...
  - ✅ (2025-11-30) Автоматизирован журнал операций: CLI `SupraLottery/supra/scripts/incident_log.sh`, Python-модуль `supra.tools.incident_log`, обновлены runbook, release checklist и инструкции с примерами.
...
- **Этап 6 — пострелизная поддержка.**
  ...
  - ✅ (2025-12-01) Подготовлены `operations/post_release_support.md` и `operations/postmortems.md`, обновлены runbook, мониторинг, статусная страница и чек-лист релиза для интеграции пострелизных процедур.
