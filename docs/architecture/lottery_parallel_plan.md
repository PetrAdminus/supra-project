...
### 3.4 События
- `LotteryCreated(event_version, id, cfg_hash, config_version, creator, event_code, series_code, run_id, primary_type, tags_mask)`
- `TicketBought(event_version, id, buyer, ticket_no, amount, sales_batch_code)`
- `SalesClosed(event_version, id, total_tickets, timestamp, closing_block)`
- `VrfRequested(event_version, id, request_id, payload_hash, schema_version, max_gas_limit, max_gas_price)`
- `VrfFulfilled(event_version, id, request_id, payload_hash, attempt, proof_hash, seed_hash)`
- `WinnerComputed(event_version, id, batch_no, winners_batch_hash, processed_count, cursor_from, cursor_to)`
- `PayoutBatch(event_version, id, batch_no, paid, total, asset_type_code, cursor_from, cursor_to, payout_nonce)`
- `PartnerPayout(event_version, id, partner_addr, payout_slot_code, asset_type_code, amount, payout_nonce)`
- `LotteryFinalized(event_version, id, archive_slot_hash, primary_type, tags_mask)`
- `LotteryCanceled(event_version, id, reason_code)`
- `PurchaseRateLimitHit(event_version, id, signer, limit_code)`
- `SalesGraceRejected(event_version, id, signer, submitted_at)`
- `AutomationDryRunPlanned(event_version, action_hash, executes_after_ts)`
- `AutomationCallRejected(event_version, action_hash, reason_code)`
- `AutomationKeyRotated(event_version, new_key_hash)`
- `PriceFeedClampTriggered(event_version, asset_id_code, old_price, new_price)`
- `VrfGasBudgetLow(event_version, id, remaining_budget)`
- `VrfRequestsPausedByDeposit(event_version, effective_balance, minimum_balance)`
- `RefundBatchStarted(event_version, id, batch_no, total_refunds)`
- `RefundBatchCompleted(event_version, id, batch_no, processed_refunds)`
- `RefundSlaBreached(event_version, id, breached_metric)`
- `GarbageCollected(event_version, id, reclaimed_slots)`
- `ArchiveDualWriteCompleted(event_version, series_code, run_id)`
...
- **View-функции для фронтенда.** Модуль `lottery_multi::views` предоставляет выборки `get_lottery_summary(id)`, `get_lottery_status(id)` (компактный агрегат `status`, `tickets_sold`, `proceeds_accum`, `vrf_status`, `primary_type`, `tags_mask`), `get_lottery_badges(id)` (возвращает `(primary_type, tags_mask)` для бейджей и подсветки на фронтенде), `list_lotteries(status, from, limit)`, `list_by_primary_type(primary_type, from, limit)`, `list_by_tag_mask(tag_mask, from, limit)` (возвращает ID лотерей, где `(tags_mask & tag_mask) != 0`), `list_by_all_tags(tag_mask, from, limit)` (возвращает ID, удовлетворяющие `(tags_mask & tag_mask) == tag_mask`), `get_user_participation(addr, from, limit)`, `get_user_rewards(addr)` и `get_partner_allowance(addr)`. Все списки следуют стабильной сортировке `id DESC` и пагинации `(from, limit)`.
