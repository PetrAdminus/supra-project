...
### 3.1 Пакеты Move
- **Новый пакет `lottery_multi`.** Он инкапсулирует модули `registry`, `views`, `history`, `roles`, `automation`. Пакет зависит от `lottery_core` (повторное использование библиотек работы с билетами, сейфами и комиссией) и `lottery_support` (существующие структуры истории и миграций). Дополнительно следует задокументировать зависимости в `SupraLottery/supra/move_workspace/Move.toml`.
- **`lottery_rewards`.** Пакет сохраняет ответственность за агрегирование наград (`Jackpot`, `Store`, `NftRewards`). Новый пакет `lottery_multi` обращается к этим модулям через чётко определённые интерфейсы (например, `reward_bridge::reserve_from_sales`, `reward_bridge::escrow_nft`).
...
- `Accounting` резервирует доли джекпота/продаж, средства для NFT и комиссию.
...
3. **Closing.** Завершение продаж, фиксация снапшота участников, подготовка данных к VRF-запросу (расчёт `payload_hash`, сохранение количества билетов).
...
6. **WinnerComputation.** Расчёт победителей вне колбэка; поддержка батчей, маппинг `winner → prize_slot`. Возможен повторный запуск при сбоях.
...
8. **Finalized/Archived.** Перенос статистики в историю, очистка оперативных структур (чанки билетов, временные таблицы), публикация `LotteryFinalized`.
...
- **Схема слотов.** Каждая лотерея хранит `vector<PrizeSlotSpec>` с идентификаторами призовых слотов, указанием типа награды и требуемым количеством победителей. Параметр `rng_count` всегда равен длине списка слотов, поэтому при создании розыгрыша выполняется проверка `rng_count == prize_slots.len()`; при необходимости нескольких случайных чисел в рамках одного слота (например, для мульти-победителей с равными долями) слот задаёт `winners_per_slot`.
- **Алгоритм сопоставления.** После получения VRF seed модуль `winner_picker::compute` конвертирует его в последовательность 256-битных слов (через SHA3-256 от `seed || slot_index`). Для каждого слота берётся следующее слово и нормализуется: `winner_index = random_word % total_tickets`. Чтобы исключить повторную выдачу одного и того же адреса на разные слоты, используется структура `UsedTicketBitmap`; при коллизии выполняется следующий хэш `SHA3-256(seed || slot_index || retry_counter)`. Таким образом, распределение остаётся равномерным и не зависит от порядка слотов.
- **Идемпотентность батчей.** Результаты вычислений пишутся в таблицу `WinnerAssignment { lottery_id, slot_id, ticket_ref, hash }` и сопровождаются событием `WinnerComputed`. Повторный запуск батча проверяет наличие записи и совпадение хэша `hash = SHA3-256(seed || slot_id || ticket_ref)`; при несоответствии транзакция завершается `abort(E_WINNER_MISMATCH)`. Порядок обработки слотов фиксируется заранее (по индексу в векторе), что гарантирует детерминизм на всех узлах.
...
- **Schema ID.** Для каждого VRF-запроса формируется структура `PayloadV1 { lottery_id, run_id, ticket_supply, prize_plan_hash, entropy_salt }`. Хэш этой структуры вычисляется как `sha3-256(bcs::to_bytes(payload))` и сохраняется в `VrfState.payload_hash`. Поле `payload_schema` содержит монотонный `u16`, позволяющий обновлять формат без потери совместимости; при деплое новой версии в миграции добавляется запись `payload_schema += 1` и описание изменений.
- **Request ledger.** В `VrfState` хранится `request_id` и флаг `consumed`. Колбэк `on_vrf_fulfilled` проверяет, что `consumed == false`, сверяет хэш payload, request_id и подпись провайдера (через официальный verifier Supra). После успешной проверки флаг выставляется в `true`, а событие `VrfFulfilled` включает `schema_id` и `payload_hash`.
- **Replay-защита.** Если колбэк приходит повторно с тем же `request_id`, но `consumed == true`, транзакция завершается `abort(E_VRF_ALREADY_CONSUMED)`. Если подпись или хэш не совпадают, выкидывается `abort(E_VRF_PAYLOAD_MISMATCH)` и публикуется событие `VrfReplayDetected`. AutomationBot и мониторинг реагируют на событие, переводя лотерею в аварийный режим.
...
### 3.4 События
- `LotteryCreated(id, cfg_hash, creator, event_slug, series_code, run_id)`
- `TicketBought(id, buyer, ticket_no, amount, sales_batch)`
- `SalesClosed(id, total_tickets, timestamp)`
- `VrfRequested(id, request_id, payload_hash, max_gas_limit, max_gas_price)`
- `VrfFulfilled(id, request_id, proof_hash, seed_hash)`
- `WinnerComputed(id, batch_no, winners_hash, processed_count)`
- `PayoutBatch(id, batch_no, paid, total, asset_type)`
- `PartnerPayout(id, partner_addr, payout_slot, asset_type, amount)`
- `LotteryFinalized(id)`
- `LotteryCanceled(id, reason_code)`

События должны обеспечивать трассировку «розыгрыш ↔ VRF ↔ победители» для аудиторов и фронтенда.
...
- **View-функции для фронтенда.** Модуль `lottery_multi::views` предоставляет выборки `get_lottery_summary(id)`, `list_lotteries(status, from, limit)`, `get_user_participation(addr, from, limit)`, `get_user_rewards(addr)` и `get_partner_allowance(addr)`. Фронтендовый раздел «История» строится поверх этих вью и событий; дополнительные SQL/JSON-хранилища разрешены только как кеши, синхронизируемые по событиям.
...
- **Роль `AutomationBot` (AutomationCap).**
  ...
  - **Ограничения.** В capability хранится расписание (`cron_spec`), список разрешённых процедур и счётчик попыток. Бот не может создавать
    ...
    тип действия.
  - **Аудит.** Каждая операция порождает `AutomationTick` с параметрами выполнения, а отклонения/ошибки логируются как `AutomationError`.
...
- **Распределение capabilities и хранение ограничений.** Все перечисленные капабилити реализуются как структуры с `drop | store`, содержащие
  ...
  - `PayoutBatchCap` — `max_batch_size`, `remaining_budget`, `cooldown_secs` между батчами.
...
- **Агрегаты по пулам.** План предусматривает расширение `LotteryPool` полями `total_allocated`, `total_prize_paid`, `total_operations_paid`, а также view-функции для выгрузки статистики. Так фронтенд и отчётность получают данные без off-chain перерасчётов.
...
- **Метрики.** Обязательные показатели: скорость продаж, очередь VRF-колбэков, длительность батчей выплат, процент успешных транзакций. Требуется список имён метрик и рекомендуемые диапазоны.
...
- **Админская консоль.** Для ролей `RootAdmin` и `OperationalAdmin` проектируется отдельный интерфейс «Конструктор лотерей» с полным доступом ко всем параметрам конфигурации (`event_slug`, `series_code`, пресет призов, лимиты билетов, окна продаж, параметры VRF). Панель подтягивает текущие `config_version`, позволяет собирать индивидуальные конфигурации, валидирует их через `lottery_multi::views::validate_config` и перед отправкой транзакции отображает дифф по сравнению с утверждёнными пресетами. Поддерживаются черновики: админ может сохранить Draft без публикации и вернуться к нему позже. Все действия завершаются транзакциями `create_custom_lottery`/`update_lottery_config`, подписываемыми кошельком администратора.
- **Партнёрский мастер.** Партнёры работают только через упрощённый мастер из заранее утверждённых шаблонов. UI показывает доступные пресеты, остаток квот (`allowance`, `max_parallel`, `expires_at`), доступные интервалы дат и автоматические ограничения (`max_ticket_price`, `max_duration`). Перед отправкой транзакции `create_from_template` фронтенд выполняет локальную валидацию и вызывает view `get_partner_template_limits(partner)` для сверки лимитов. Если лимиты превышены, интерфейс блокирует шаг и показывает ссылку на процедуру продления через `RootAdmin`; автоматизация через governance добавится на позднем этапе.
...
- **Fallback и проверки.** План предусматривает резервный агрегатор: при недоступности основного фида система активирует fallback, публикует событие `PriceFeedFallbackActivated` и уведомляет фронтенд. Все операции проверяют `staleness_window`, допустимые диапазоны значений и соответствие конфигурациям лотерей.
...
- **Совместимость и мониторинг.** Автоматизация (боты, индексаторы) при запуске проверяют три слоя: git-релиз, версию Move-пакета и `config_version` активных лотерей. Несовпадение трактуется как требование к миграции и фиксируется событием `VersionDriftDetected` для дальнейшего аудита.
