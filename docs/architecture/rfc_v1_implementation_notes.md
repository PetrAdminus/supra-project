...
| 5    | Наблюдаемость, автоматизация, прайс-фиды, очистка хранилища                    | In Progress        | Расширен runbook AutomationBot (dry-run, таймлоки, контроль `failure_count`), подготовлены `operations/release_checklist.md`, `operations/monitoring.md`, `operations/incident_log.md`; модуль `price_feed` получил тесты `price_feed_tests`, Prover-спеку и справочник `price_feeds.md`; добавлены Move-тесты `automation_tests` для `ensure_action` и сброса лимитов; view `status_overview` агрегирует статусы, VRF и выплаты для статусной страницы, покрыта тестом `views_tests::status_overview_counts_vrf_and_statuses` и JSON Schema v1.0.2 | 2025-11-27 |
