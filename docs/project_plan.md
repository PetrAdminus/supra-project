...
- Модуль `lottery_rewards::nft_rewards` уже выпускает уникальные бейджи победителей: события `BadgeMintedEvent`/`BadgeBurnedEvent`, view-функции `has_badge`/`list_badges`/`get_badge` и Move-тесты подтверждают корректность минта, ограничение по администратору и возможность ручного сжигания владельцем.
