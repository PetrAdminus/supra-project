...
## Решение
1. **Встроенный выпуск бейджей в Move.** Добавлен модуль `lottery::nft_rewards`, размещённый под адресом @lottery. Он хранит глобальное состояние (администратор, счётчик бейджей, таблица коллекций пользователей), генерирует события `BadgeMintedEvent`/`BadgeBurnedEvent` и предоставляет view-функции `has_badge`, `list_badges`, `get_badge` для оффчейн-сервисов.
...
4. **Unit-тесты.** Файл `lottery/tests/nft_rewards_tests.move` подтверждает успешный выпуск, невозможность минта неадминистратором и ручное сжигание владельцем. Это фиксирует контрактный интерфейс до интеграции с фронтендом.
