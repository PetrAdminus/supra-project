...
| `lottery::autopurchase` | `lottery_rewards` | 🟢 Перенесено | Полная логика автопокупок перенесена, операции используют capability раундов и казначейства ядра. |
| `lottery::jackpot` | `lottery_rewards` | 🟢 Перенесено | Использует `MultiTreasuryCap` (`scope = SCOPE_JACKPOT`) через ресурс `JackpotAccess`, логика розыгрыша и события сохранены. |
| `lottery::referrals` | `lottery_rewards` | 🟢 Перенесено | Полная логика реферальной программы перенесена; выплаты используют `ReferralsControl` с `MultiTreasuryCap`, снапшоты и события восстановлены. |
| `lottery::store` | `lottery_rewards` | 🟢 Перенесено | Полная логика магазина перенесена; операции используют `MultiTreasuryCap` (`StoreAccess`) и фиксируют события как в монолите. |
| `lottery::vip` | `lottery_rewards` | 🟢 Перенесено | Полная логика VIP-подписок перенесена в пакет наград, бонусы используют `MultiTreasuryCap`. |
...
**Статус:** ✅ Завершено (обновлено 2025-10-30).
...
|---|---|---|---|---|
| `lottery_core` | `lottery/sources/Lottery.move`, `LotteryRounds.move`, `LotteryInstances.move`, `Treasury.move`, `TreasuryMulti.move`, `Operators.move` | `supra/move_workspace/lottery_core/sources/<Module>.move` | Требуется сохранить порядок инициализации и `CoreControl` ресурсы. | 🟢 Перенесено: вся логика раундов, казначейства и capability завершена, очереди истории/покупок и smoke-тесты активны |
...
- [x] Подготовить заглушки `sources/*.move` с декларациями модулей и комментариями TODO.
 - [x] Добавить минимальные smoke-тесты (например, `tests/rounds_queue_tests.move` проверяет очереди и выплаты в `lottery_core::rounds`).
...
- [x] Реализовать ресурс `HistoryWarden`, который кэширует `HistoryWriterCap`, и добавить `caps_ready`/`release_caps` для контроля выдачи.
- [x] Реализовать ресурс `MigrationSession`, удерживающий `InstancesExportCap` и `LegacyTreasuryCap`, с функциями `ensure_caps_initialized`/`caps_ready`/`release_caps` для миграции.
- [x] Добавить очередь результатов розыгрышей в `lottery_core::rounds` и entry `history::sync_draws_from_rounds`, чтобы операциям не приходилось передавать данные победителя вручную.
- [x] Настроить очередь покупок в `lottery_core::rounds` и синхронизацию `lottery_rewards::rounds_sync::sync_purchases_from_rounds`, чтобы VIP и реферальные выплаты выполнялись без friend-зависимостей.
- [x] Добавить скрипт `supra/scripts/sync_lottery_queues.sh` для одновременного запуска `lottery_support::history::sync_draws_from_rounds` и `lottery_rewards::rounds_sync::sync_purchases_from_rounds` (лимиты и `skip` документированы в runbook).
- [x] Обновить тестовый план: smoke-проверки миграции (описано в шаге 7).
- [x] Добавить unit-тест синхронизации истории (`tests/history_sync_tests.move`) и задокументировать очередь раундов.
- [x] Добавить smoke-тесты миграции (`tests/migration_tests.move`), проверяющие жизненный цикл `MigrationSession` и перенос состояния.
...
- [x] Зафиксировать тесты на изоляцию scope в плане (см. шаг 7).
- [x] Проверить, что сборка проходит на заглушках (Aptos CLI подтвердил компиляцию всех модулей).
- [x] Добавить модуль `rounds_sync`, который выгружает очередь покупок из ядра, начисляет VIP-бонусы и делегирует суммы в `referrals::record_reward`.

##### Состояние каркасов (2025-10-28)

- **lottery_core** — созданы каталоги и `Move.toml`, перенесены `main_v2`, `operators`, `instances`, `treasury_v1` и `treasury_multi` из монолита. `main_v2` теперь живёт в пакете ядра, сохраняет события и admin-гард для миграции (`export_state_for_migration(caller)`/`clear_state_after_migration(caller)`); `lottery_core::rounds` разворачивает `CoreControl` и управляет `HistoryWriterCap`/`AutopurchaseRoundCap`; `treasury_v1` реализует mint/burn/transfer и выдачу capability (`AutopurchaseTreasuryCap`, `LegacyTreasuryCap`, `payout_with_*`); `treasury_multi` включает полную логику распределения, события, view и guarded API (`borrow/return_multi_treasury_cap`, `cap_available`, `scope_*`). Из заглушек осталась только `rounds::migrate_import_round`, которая будет заполнена вместе с переносом логики раундов. Unit-тест `tests/rounds_queue_tests.move` проверяет прохождение розыгрыша, заполнение очередей и выплату приза через `treasury_v1`/`treasury_multi`.
- **lottery_support** — модули `metadata`, `history` и `migration` перенесены из монолита; `history::record_draw` использует кэшированный `HistoryWriterCap` в ресурсе `HistoryWarden` (`ensure_caps_initialized`/`caps_ready`/`release_caps`), а `migration` переключён на API `lottery_core`, удерживает `InstancesExportCap` и `LegacyTreasuryCap` внутри `MigrationSession` и собирается на заглушках capability. Добавлены тесты `tests/history_sync_tests.move` и `tests/migration_tests.move`, которые покрывают синхронизацию очереди `lottery_core::rounds` и миграцию с кэшированием capability.
- Добавлена потокобезопасная очередь результатов розыгрышей в `lottery_core::rounds`; администраторы могут вызывать `lottery_support::history::sync_draws_from_rounds(limit)` для пакетной записи истории вместо ручного ввода параметров события.
- Реализована очередь покупок `lottery_core::rounds` → `lottery_rewards::rounds_sync`: `sync_purchases_from_rounds` выдаёт бесплатные VIP-билеты через `grant_bonus_tickets_admin` и передаёт сумму покупки в `referrals::record_reward`, сохраняя автоматическое начисление наград без friend-зависимостей. Для оперативной синхронизации администраторы могут запускать `supra/scripts/sync_lottery_queues.sh`, который последовательно вызывает обе очереди с настраиваемыми лимитами.
- ✅ Добавлен smoke-тест `lottery_rewards::rounds_sync_tests`, который проверяет выгрузку очереди покупок, начисление VIP-бонусов и реферальных выплат через `rounds_sync::sync_purchases_from_rounds`.
- ✅ Перенесён smoke-тест `lottery_rewards::autopurchase_tests`, который проходит настройку плана, депозит, последовательные `execute` и возврат средств через capability `AutopurchaseRoundCap`/`AutopurchaseTreasuryCap`.
- **lottery_rewards** — модуль `nft_rewards` перенесён с полной логикой и тестами; `autopurchase` перенесён из монолита, удерживает ресурс `AutopurchaseAccess`, использует capability `AutopurchaseRoundCap`/`AutopurchaseTreasuryCap` для закупки и возвратов, предоставляет `caps_ready`/`release_caps`; `jackpot` перенесён полностью и использует `JackpotAccess` для кеширования `MultiTreasuryCap` (`scope = SCOPE_JACKPOT`), сохранив события, снапшоты и VRF-флоу; `referrals` перенесён целиком — ресурс `ReferralsControl` кеширует `MultiTreasuryCap` (`scope = SCOPE_REFERRALS`), восстановлены события, снапшоты и расчёт выплат через `record_reward`; `store` перенесён полностью — `StoreAccess` кэширует cap со scope `SCOPE_STORE`, `purchase` записывает доходы через `record_operations_income_with_cap`; `vip` перенесён полностью — `VipAccess` лениво кеширует cap (`scope = SCOPE_VIP`), подписки сохраняют бонусные билеты и учёт выданных бонусов. Добавлены unit-тесты `tests/multi_treasury_scope_tests.move`, которые проверяют изоляцию scope и корректное освобождение capability между модулями наград.
...
| `lottery_rewards` | ``MOVE_CLI=/tmp/aptos-cli/aptos PYTHONPATH_OVERRIDE=SupraLottery bash supra/scripts/build_lottery_packages.sh lottery_rewards`` | ✅ Успех | Пакет собирается с перенесённой логикой автопокупок, зависимости `lottery_core` подтянуты из кеша. |
...
| `lottery::rounds` | `lottery_core::rounds` | Полная логика перенесена: очереди истории и покупок, выдача `HistoryWriterCap`/`AutopurchaseRoundCap`, API синхронизации для наград. | 🟢 Перенесено | Очереди выгружаются через `lottery_support::history::sync_draws_from_rounds` и `lottery_rewards::rounds_sync::sync_purchases_from_rounds`, покрыто smoke-тестами `rounds_queue_tests` и `rounds_sync_tests`. |
...
| `lottery::history` | `lottery_support::history` | Перенести записи розыгрышей, перевести `record_draw` на `HistoryWriterCap`. | 🟢 Перенесено | Выдача `HistoryWriterCap` реализована в `lottery_core::rounds`; добавлен entry-хук `record_draw_from_rounds` для вызова администратором сразу после `fulfill_draw_request`. |
...
| `lottery::vip` | `lottery_rewards::vip` | Перенести модуль VIP-подписок, кешируя `MultiTreasuryCap (scope = SCOPE_VIP)` и сохраняя выдачу бонусных билетов. | 🟢 Перенесено | Бонусы выдаются через очередь покупок (`rounds_sync::sync_purchases_from_rounds`), smoke-тест `vip_tests` подтверждает подписку, начисление и отмену. |
| `lottery::referrals` | `lottery_rewards::referrals` | Полностью перенесён: `ReferralsControl` кеширует `MultiTreasuryCap` (`scope = SCOPE_REFERRALS`), entry `record_reward` выполняет выплаты, восстановлены события, снапшоты и тестовые view. | 🟢 Перенесено | Интеграция с очередью покупок завершена (`rounds_sync`), smoke-тест `referrals_tests` проверяет выплаты и снапшоты. |
| `lottery::store` | `lottery_rewards::store` | Перенести модуль магазина, адаптировав учёт доходов к `MultiTreasuryCap` и кешу capability. | 🟢 Перенесено | Smoke-тест `store_tests` покрывает склад, события и пополнение операционного пула через `record_operations_income_with_cap`. |
| `lottery::autopurchase` | `lottery_rewards::autopurchase` | Полная логика перенесена: планы игроков, события и операции работают через capability раундов и казначейства; `on_draw_fulfilled` будет включён вместе с переносом наград. | 🟢 Перенесено | Ждём миграции `jackpot`/`referrals` для финальных интеграций. |
| `lottery::jackpot` | `lottery_rewards::jackpot` | Полная логика перенесена: розыгрыши через VRF, события, снапшоты и распределение приза работают через `MultiTreasuryCap` (`JackpotAccess`). | 🟢 Перенесено | Smoke-тест `jackpot_tests` подтверждает жизненный цикл VRF-запроса и распределение приза. |
...
- ✅ Перенесён модуль `lottery::history` в `lottery_support::history`, `record_draw` требует `HistoryWriterCap` вместо friend-доступа, добавлен entry-хук `record_draw_from_rounds` для VRF-агрегатора.
- ✅ Перенесён модуль `lottery::migration` в `lottery_support::migration`; код повторяет функциональность монолита, а выдача `InstancesExportCap` и `LegacyTreasuryCap` теперь оформлена через ресурс `MigrationSession` с функциями `ensure_caps_initialized`/`caps_ready`/`release_caps`.
- ✅ Добавлены smoke-тесты `lottery_support::migration_tests`, которые проверяют выдачу/возврат capability в `MigrationSession` и корректный импорт состояния через `migrate_from_legacy`.
...
- ✅ Перенесён модуль `lottery::store` в `lottery_rewards::store`: ресурс `StoreAccess` кеширует `MultiTreasuryCap` (`scope = SCOPE_STORE`), `purchase` ведёт учёт доходов через `record_operations_income_with_cap`, восстановлены события и snapshot/view API.
...
- ✅ Перенесена бизнес-логика `lottery::autopurchase` в `lottery_rewards::autopurchase`: сохранены планы игроков, события, snapshot/view API и операции `configure_plan`/`deposit`/`execute`/`refund`, которые используют `rounds::record_prepaid_purchase` и `treasury_v1::payout_with_autopurchase_cap` через кэшированные capability.
- ✅ Перенесён модуль `lottery::jackpot` в `lottery_rewards::jackpot`: восстановлены VRF-запросы, события, снапшоты и распределение приза через `treasury_multi::distribute_jackpot_with_cap`, capability кешируется в ресурсе `JackpotAccess` с ленивой инициализацией/возвратом.
- ✅ Перенесён модуль `lottery::referrals` в `lottery_rewards::referrals`: `ReferralsControl` кеширует `MultiTreasuryCap`, восстановлены события регистрации/выплат, снапшоты и вспомогательные view; entry-функция `record_reward` оформляет выплаты из операционного пула.
- ✅ Добавлены smoke-тесты `jackpot_tests`, `referrals_tests`, `store_tests`, `vip_tests`, которые покрывают VRF-розыгрыш, синхронизацию очередей покупок (`rounds_sync`), пополнение операционного пула и управление подписками.
...
- ✅ Подготовлен документ `docs/architecture/modules.md`, фиксирующий capability API, обработку очередей и распределение smoke-тестов между пакетами.

Подготовительные задачи перед стартом шага 5:

- [x] Зафиксировать финальные сигнатуры capability API (`borrow_*_cap`) в `lottery_core` и задокументировать их в `docs/architecture/modules.md`.
- [x] Подготовить шаблоны smoke-тестов для каждого пакета (см. сценарии шага 7) и определить минимальный набор проверок после переноса.
...
- [x] После прохождения первичной сборки — перейти к переносу фактического кода модулей и обновлению runbook (шаг 5).
...
**Статус:** ✅ Завершено (обновлено 2025-10-31).

- [x] В `docs/testnet_runbook.md` добавить команды публикации для каждого нового пакета и порядок вызова `ensure_caps_initialized`.
- [x] В `README.md` описать структуру `lottery_core`/`lottery_support`/`lottery_rewards`, обновить инструкции по тестам и раздел казначейства.
- [x] Актуализировать остальные runbook/гайды (deployment checklist, frontend-инструкции) под новую структуру пакетов.

> 2025-10-31: `SupraLottery/docs/testnet_deployment_checklist.md` переведён на алиасы `lottery_core`/`lottery_support`/`lottery_rewards`, добавлены шаги синхронизации очередей и резервирование capability через `ensure_caps_initialized`/`release_caps`. `frontend/docs/supra_integration.md` расширен переменными окружения для новых пакетов и инструкциями по вызову RPC из `lottery_support` и `lottery_rewards`.

### 6. Пошаговая публикация

**Статус:** 🟡 В работе (обновлено 2025-11-01).
...
> 2025-11-01: добавлен скрипт `supra/scripts/publish_lottery_packages.sh`, который автоматизирует последовательный `move tool publish` для всех пакетов. Runbook и чек-лист тестнета дополнены примером запуска, переменными окружения (`SUPRA_PUBLISH_EXTRA_ARGS`, `SUPRA_PUBLISH_PAUSE`) и пометкой о новой автоматизации. Следующий шаг — зафиксировать команды инициализации capability после деплоя и подготовить шаблон отчёта по транзакциям.
...
**Статус:** ⏸ Отложено (не выполнять без отдельного запроса).

> Сервис всё ещё в активной разработке, простоев не ожидается, поэтому шаг выполняется только при специальном решении ответственных.

- При необходимости возврата к монолиту:
...
- Документацию по восстановлению поддерживать актуальной, но без запуска практических процедур до отдельного запроса.
