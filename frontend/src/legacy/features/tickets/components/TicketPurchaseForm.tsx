...
export function TicketPurchaseForm({ lotteryId, round, ticketPrice }: TicketPurchaseFormProps): ReactElement {
  ...
  return (
    <form className="ticket-form" onSubmit={handleSubmit}>
      ...
      <div className="ticket-form__row ticket-form__row--footer">
        <span className="ticket-form__price">{priceLabel}</span>
        <button className="ticket-form__submit" type="submit" disabled={submitDisabled}>
          <Plus size={18} />
      ...
      {mutation.isError && (
        <p className="ticket-form__error">
          {mutation.error instanceof Error ? mutation.error.message : t("tickets.form.errorInvalidNumbers")}
        </p>
      )}
