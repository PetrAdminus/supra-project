...
1. Для преобразования `&mut T` → `&T` используем ре-заимствование `&*value`: в поставке Supra Move пока нет стандартного модуля `std::borrow`, описанного в Move 2024, поэтому `borrow::freeze` недоступен.
...
Используем явное ре-заимствование `let snapshot = build_snapshot(&*state);`. Такой приём эквивалентен `borrow::freeze`, но не требует модуля `std::borrow`.
