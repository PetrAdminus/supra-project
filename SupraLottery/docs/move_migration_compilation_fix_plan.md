...
1. Для преобразования `&mut T` → `&T` добавлен локальный shim `std::borrow::freeze` (см. `lottery/sources/Borrow.move`), повторяющий поведение одноимённой функции из Move 2024. Используем его вместо прямого ре-заимствования `&*value`.
...
Используем `borrow::freeze(state)` из локального shim-модуля `std::borrow`, чтобы получить неизменяемую ссылку без требования `copy` у структуры.
