...
- Клиент Supra dVRF v3 (модули `supra_vrf`, `deposit`, VRF-хаб `vrf_hub::hub` и мульти-лотерейные компоненты `lottery_core::instances`/`lottery_core::rounds`)
- Казначейство: Fungible Asset-токен через `lottery_core::treasury_v1` и многопуловое распределение через `lottery_core::treasury_multi`
...
Тест проверяет соответствие расчётов `calc_min_balance.calculate` формуле из `lottery_core::main_v2`, корректность разбора чисел с разделителями (`parse_u128`) и форматирование человекочитаемых сумм.
...
### Автопокупка билетов
Модуль `lottery_rewards::autopurchase` хранит планы игроков на автоматическую покупку билетов и позволяет оффчейн-сервисам выполнять их без повторного участия пользователя. Основные сценарии:
...
`lottery_support::migration` обеспечивает перенос накопленного состояния из исторической версии контракта в актуальные таблицы мульти-лотереи и теперь фиксирует полный снапшот операции:
...
### VIP-подписки
Модуль `lottery_rewards::vip` добавляет платные подписки, которые дают игрокам дополнительные билеты при каждой покупке и пополняют операционный пул:
...
3. **Бонусные билеты** — после пользовательских или автоматических покупок администратор запускает `lottery_rewards::rounds_sync::sync_purchases_from_rounds`: функция извлекает очередь покупок из `lottery_core::rounds`, добавляет бесплатные билеты через `grant_bonus_tickets_admin`, фиксирует `VipBonusIssuedEvent` и передаёт объём покупки в `referrals::record_reward`, сохраняя прежнюю автоматизацию начислений без friend-зависимостей.
...
Модуль `lottery_rewards::store` реализует продажу цифровых активов (например, кастомных аватаров или NFT-скинов), поступления от которой пополняют операционный пул лотереи:
...
Модуль `lottery_support::metadata` хранит описания розыгрышей, которые нужны фронтенду, промо-сайту и партнёрам без обращения к внешнему CMS:
...
Модуль `lottery_core::operators` фиксирует владельцев отдельных розыгрышей и выданные им делегированные роли:
...
### История розыгрышей
Модуль `lottery_support::history` сохраняет результаты розыгрышей и предоставляет API для панели честности и аналитики:
...
2. **Запись результатов** — `lottery_core::rounds` складывает каждый исполненный розыгрыш в очередь `HistoryQueue`, которую администратор выгружает через `lottery_support::history::sync_draws_from_rounds`. Хук повторно использует `record_draw`, публикуя `DrawRecordedEvent` с теми же полями (победитель, билет, сумма приза, байты случайности и payload) и сохраняя лимит из 128 записей.
...
Для обслуживания обеих очередей предусмотрен скрипт `supra/scripts/sync_lottery_queues.sh`: он последовательно вызывает `lottery_support::history::sync_draws_from_rounds` и `lottery_rewards::rounds_sync::sync_purchases_from_rounds`, принимая профиль Supra CLI и лимиты (`0` — обработать все записи, `skip` — пропустить соответствующий шаг).

### Реферальные бонусы
Модуль `lottery_rewards::referrals` позволяет гибко настраивать бонусы за приглашение и автоматически выплачивать их из операционного пула:
...
3. **Автовыплаты при покупке** — `lottery_rewards::rounds_sync::sync_purchases_from_rounds` выгружает очередь из `lottery_core::rounds`, начисляет VIP-бонусы и передаёт объём покупки в `lottery_rewards::referrals::record_reward`, где сумма списывается из операционного пула через `lottery_core::treasury_multi::pay_operations_bonus_with_cap` и фиксируется событием `ReferralRewardPaidEvent`.
...
Модуль `lottery_rewards::nft_rewards` позволяет отмечать победителей уникальными NFT-бейджами:

1. `nft_rewards::init` развёртывает глобальное состояние под адресом @lottery (alias `@lottery_rewards`), публикует счётчик бейджей и события `BadgeMintedEvent`/`BadgeBurnedEvent`.
2. `nft_rewards::mint_badge` может вызываться только администратором (адрес @lottery_rewards, совпадающий с прежним @lottery); она резервирует новый `badge_id`, сохраняет метаданные (`lottery_id`, `draw_id`, `metadata_uri`, адрес минтера) и эмитит событие, пригодное для панели честности и фронтенда.
...
4. Для мониторинга доступны `has_badge`, `list_badges`, `list_owner_addresses`, `get_owner_snapshot` и `get_snapshot`: они возвращают как индивидуальные бейджи, так и агрегированные снимки (администратор, счётчик `next_badge_id`, владельцы и их награды). Событие `NftRewardsSnapshotUpdatedEvent` публикуется после `init`, минта и бёрна, поэтому Supra и аудиторы получают актуальный снимок без реконструкции истории событий. Move-тесты пакета `lottery_rewards` (`nft_rewards_tests.move`) покрывают выпуск, запрет минта неадминистратором, бёрн владельцем и корректность snapshot/view/событий.
...
#### Коллекция экземпляров `lottery_core::instances`
...
- Для реферальных бонусов добавлен внутренний помощник `pay_operations_bonus_internal`, списывающий часть операционного пула и фиксирующий событие `OperationsBonusPaidEvent`; его используют модули роста (`lottery_rewards::referrals`) и мониторинг.
...
#### Глобальный джекпот `lottery_rewards::jackpot`
...
#### Миграция со старого контракта `lottery_support::migration`
- `migrate_from_legacy` переносит состояние из `lottery_core::main_v2`: копирует список билетов, призовой пул, флаг расписания и обновляет таблицы `instances`, `rounds`, `treasury_multi`.
