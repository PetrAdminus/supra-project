"""Конфигурация параметров газа для Supra VRF через CLI."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Настроить max_gas_price/max_gas_limit/verification_gas_value для Supra VRF",
    )
    ...
    parser.add_argument(
        ...
        help="значение callback_gas_price (u128) для configure_vrf_gas",
    )
    parser.add_argument(
        "--callback-gas-limit",
        type=int,
        default=env_default("CALLBACK_GAS_LIMIT", int),
        help="значение callback_gas_limit (u128) для configure_vrf_gas",
    )
    parser.add_argument(
        "--assume-yes",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<lottery_addr>::core_main_v2::configure_vrf_gas"
        ),
    )
    return parser
...
def build_command_args(ns: argparse.Namespace) -> List[str]:
    ...
    return [
        ...
        f"u128:{callback_gas_price}",
        f"u128:{callback_gas_limit}",
        f"u128:{verification_gas}",
    ]
...
def target_function_id(ns: argparse.Namespace) -> str:
    ...
    return f"{ns.lottery_addr}::core_main_v2::configure_vrf_gas"
