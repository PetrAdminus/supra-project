#!/usr/bin/env python3
"""Encode LegacyInstanceRecord payloads and optionally execute the import."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate a JSON InstanceRegistry snapshot, encode it as vector<LegacyInstanceRecord> "
            "and optionally run lottery_data::instances::import_existing_instances."
        )
    )
    parser.add_argument(
        "snapshot",
        help=(
            "Path to JSON file with instance data. Expected format: "
            '{"records": [{"lottery_id": 1, "owner": "0x...", "lottery_address": "0x...", '
            '"ticket_price": 0, "jackpot_share_bps": 0, "tickets_sold": 0, '
            '"jackpot_accumulated": 0, "active": true}, ...]}'
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--output-bcs",
        default="tmp/instance_records_import.bcs",
        help="Where to write the encoded vector<LegacyInstanceRecord> payload.",
    )
    parser.add_argument(
        "--function",
        default="lottery_data::instances::import_existing_instances",
...
def encode_record(record: Dict[str, Any], index: int) -> bytearray:
    ...
    buffer.extend(encode_address(record.get("owner"), f"records[{index}].owner"))
    buffer.extend(
        encode_address(record.get("lottery_address"), f"records[{index}].lottery_address")
    )
    buffer.extend(encode_u64(record.get("ticket_price"), f"records[{index}].ticket_price"))
    buffer.extend(
        encode_u16(record.get("jackpot_share_bps"), f"records[{index}].jackpot_share_bps")
    )
    buffer.extend(encode_u64(record.get("tickets_sold"), f"records[{index}].tickets_sold"))
    buffer.extend(
        encode_u64(record.get("jackpot_accumulated"), f"records[{index}].jackpot_accumulated")
    )
    buffer.extend(encode_bool(record.get("active"), f"records[{index}].active"))
    return buffer
...
def main() -> int:
    ...
    print(
        f"Encoded {len(records)} instance record(s) to {output_path} (size={len(payload)} bytes)",
        flush=True,
    )
