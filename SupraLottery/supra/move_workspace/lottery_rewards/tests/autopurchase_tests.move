...
module lottery_rewards::autopurchase_tests {
    ...
    fun executes_autopurchase_plan(
        ...
        test_utils::assert_len_eq<autopurchase::AutopurchaseExecutedEvent>(
            &execute_events_first,
            1,
            177,
        );
        let first_execute_event = *test_utils::last_event_ref(&execute_events_first);
        let autopurchase::AutopurchaseExecutedEvent {
            lottery_id: first_execute_lottery,
            player: first_execute_player,
            tickets_bought: first_execute_tickets,
            spent_amount: first_execute_spent,
            remaining_balance: first_execute_remaining,
        } = first_execute_event;
        ...
        let execute_events_second =
            test_utils::drain_events<autopurchase::AutopurchaseExecutedEvent>();
        test_utils::assert_len_eq<autopurchase::AutopurchaseExecutedEvent>(
            &execute_events_second,
            1,
            193,
        );
        let second_execute_event = *test_utils::last_event_ref(&execute_events_second);
        let autopurchase::AutopurchaseExecutedEvent {
            lottery_id: second_execute_lottery,
            player: second_execute_player,
            tickets_bought: second_execute_tickets,
            spent_amount: second_execute_spent,
            remaining_balance: second_execute_remaining,
        } = second_execute_event;
        ...
        let execute_snapshot_events_second =
            test_utils::drain_events<autopurchase::AutopurchaseSnapshotUpdatedEvent>();
        test_utils::assert_len_eq<autopurchase::AutopurchaseSnapshotUpdatedEvent>(
            &execute_snapshot_events_second,
            1,
            199,
        );
        ...
        let autopurchase::AutopurchaseRefundedEvent {
            ...
            remaining_balance: refund_remaining,
        } = refund_event;
